...
with tf.Session() as sess:
    ...
    writer = tf.summary.FileWriter('/tmp/tensorflow/13examples/linear_regression/', sess.graph)
    # Fit all training data
    for epoch in range(training_epochs):
        sess.run(optimizer, feed_dict={X: train_X, Y: train_Y})
        if (epoch + 1) % display_step == 0:
            events = sess.run(merged, feed_dict={X: train_X, Y: train_Y})
            writer.add_summary(events, epoch)
